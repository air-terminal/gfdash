...
| ドキュメント | 内容 |
| :--- | :--- |
| [AI予測・レポート](./docs/ai_forecast.md) | 推論エンジンの用意、`.env` の設定、予測と所見の使い方 |
| [運用コマンド](./docs/commands.md) | 天候取得・来場者取り込み・カレンダーの各コマンド |
| [データベース総合仕様書](./docs/db_schema_index.md) | ER図、テーブル定義（TA系 / TB系 / TZ系） |
| [コード・定数定義書](./docs/codes.md) | 各種マスタコードの定義 |
| [所見解析のモデル比較](./docs/bench_remark_parse.md) | ローカルLLMの選定に使う測定例 |
...
1. 環境変数の設定
   1. `.env.example` をコピーして `.env` を作成し、環境に合わせて修正してください。
      ...
      この3項目が空のままだと、`docker compose up` は `required variable ... is missing a value`
      と表示してコンテナを作る前に止まります。値を埋めて再実行してください。

      > **以前の版で空のまま起動してしまった場合**
      > `container is unhealthy` で止まり、値を埋めても復旧しません。データベースの
      > ボリュームに初期化途中の状態が残っているためです。`docker compose down -v` で
      > ボリュームを消してから起動し直してください（初回起動前なので消えるデータはありません）。
      >
      > Windows PowerShell 5.1 で `.env` の内容を確認するときは `Get-Content .env -Encoding UTF8`
      > としてください。既定では UTF-8 を正しく読めません。
...
3. マスタデータの設定  
   ...
   **画面別パーミッションの設定**  
      画面別にユーザー権限レベルを設定します。  
      設定値については、[GFdash コード・定数定義書](./docs/codes.md)を参照してください。  
...
**6. AI予測・レポートのセットアップ（任意）**  
来場者予測とレポート生成には、ローカルLLMの推論エンジンが必要です。
用意しなくても、集計・分析の各画面はそのまま使えます。

手順と設定項目は **[docs/ai_forecast.md](docs/ai_forecast.md)** にまとめています。

* 推論エンジンの用意（Ollama / OpenAI互換API）
* `.env` の設定項目と実行パラメータのプリセット
* 予測・レポートの実行（画面／コマンド）
* 月次所見、予測の反映先、実行履歴の比較、精度の検証
...
引数と使用例は **[docs/commands.md](docs/commands.md)** を参照してください。

| コマンド | 用途 |
| :--- | :--- |
| `get_daily_weather` | 気象庁から天候情報を取得する |
| `import_attendance` | 来場者数のCSVを取り込む |
| `import_holiday2` / `export_holiday2` | 第2休日カレンダーの取り込み・書き出し |
| `gen_weather_station_master` | 気象観測地点マスタを再生成する（通常は不要） |

AI予測・レポートのコマンド（`run_forecast` / `run_llm_analysis` /
`evaluate_forecast` / `bench_remark_parse`）は
[docs/ai_forecast.md](docs/ai_forecast.md) にまとめています。
