...
- **AI機能:** 過去の来場者数・気温の傾向より来場者数を予測する機能、及びローカルLLMより当月の振り返り、翌月の予測レポートを作成します。推論エンジンは Ollama と OpenAI互換APIの2方式に対応しています。
- **月次所見:** 競合店の開業や近隣の工事など、実績データに現れる前から分かっている出来事を登録し、予測とレポートに反映できます。自由文をAIに読み取らせて補正の形に変換できるほか、手入力にも対応しているため、AIを利用できない環境でも使えます。実行条件と結果は履歴として残り、補正の前後を比較できます。
