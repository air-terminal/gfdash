from django.shortcuts import render
from django.http import HttpResponse
from datetime import datetime, timedelta, timezone, date
from dateutil.relativedelta import relativedelta
from django.db import connection

from .. import views
from ..models import Ta215Attnd
from ..models import Ta220Memo
from ..models import Tz901ComName
from ..utils.com_utils import HALF_YEAR_KAMIKI, HALF_YEAR_SIMOKI
from ..utils.com_utils import com_get_fiscal_year_sql
from ..utils.com_utils import com_get_half_year_months

import json
import calendar


def post115_main(request):

    from django.http import QueryDict
    dic = QueryDict(request.body, encoding='utf-8')
    ret = {}

    tmpParam = dic.get('getMode')
    if tmpParam == 'get':
        dictParam =  sub115_conv_param(dic)
        ret = sub115_get_raijyo_data(dictParam)
    elif tmpParam == 'init':
        dictParam =  sub115_get_init(dic)
        ret = sub115_get_raijyo_data(dictParam)
        ret['firstDay'] = format(dictParam['firstDay'],"%Y/%m/%d 00:00:00")
        ret['lastDay'] = format(dictParam['lastDay'],"%Y/%m/%d 00:00:00")
    else:
        dictParam =  sub115_get_init(dic)
        ret = sub115_get_raijyo_data(dictParam)
        ret['firstDay'] = dictParam['firstDay']
        ret['lastDay'] = dictParam['lastDay']

    return json.dumps(ret, ensure_ascii=False, indent=2)


def sub115_get_init(dic):
    #初期処理時、DB上の最新年のデータを取得する
    ta215 = Ta215Attnd.objects.all().order_by('business_day').reverse().first()
    firstRecTa215 = Ta215Attnd.objects.all().order_by('business_day').first()

    tmpParam = {}

    tmpParam['yyyy'] = int(format(ta215.business_day,"%Y"))
    tmpParam['from'] = datetime.date(datetime.strptime(format(ta215.business_day,"%Y/01/01 00:00:00"), "%Y/%m/%d %H:%M:%S"))
    tmpParam['to'] = tmpParam['from'] + relativedelta(years=1)
    tmpParam['firstDay'] = datetime.date(datetime.strptime(format(firstRecTa215.business_day,"%Y/01/01 00:00:00"), "%Y/%m/%d %H:%M:%S"))
    tmpParam['lastDay'] = datetime.date(datetime.strptime(format(ta215.business_day,"%Y/01/01 00:00:00"), "%Y/%m/%d %H:%M:%S"))
    tmpParam['chart_mode'] = dic['getChartMode']

    return tmpParam

def sub115_conv_param(dic):
    #初期処理時、DB上の最新月のデータを取得する

    tmpFrom = datetime.date(datetime.strptime(dic.get('getY'), "%Y/01/01 00:00:00"))
    tmpLastDay = calendar.monthrange(tmpFrom.year, tmpFrom.month)[1]
    tmpTo = date(tmpFrom.year, tmpFrom.month, tmpLastDay)

    tmpY, tmpM, tmpOther = dic['getY'].split('/')

    tmpParam = {}
    tmpParam['yyyy'] = int(tmpY)
    tmpParam['from'] = tmpFrom
    tmpParam['to'] = tmpTo
    tmpParam['chart_mode'] = dic['getChartMode']

    return tmpParam


def sub115_get_raijyo_data(dictParam):

    dictCtx = {}

    dictCtx['txtHeader'] = format(dictParam['from'],"%Y年")
    dictCtx['initYMD'] = format(dictParam['from'],"%Y/%m/%d 00:00:00")

    #今年のデータ
    dictCtx['yearTable'] = sub115_get_Ta215(dictParam.get('chart_mode'), dictParam['yyyy'])
    #昨年のデータ
    dictCtx['oldYearTable'] = sub115_get_Ta215(dictParam.get('chart_mode'), (dictParam['yyyy'] - 1))

    allSum = 0
    allMember = 0
    allVisitor = 0
    allSchool = 0
    allInternalSchool = 0

    for tmpTable in dictCtx['yearTable'].values():
        allSum += tmpTable['all']
        allMember += tmpTable['member']
        allVisitor += tmpTable['visitor']
        allSchool += tmpTable['school']
        allInternalSchool += tmpTable['int_school']

    dictCtx['allSum'] = allSum
    dictCtx['allMember'] = allMember
    dictCtx['allVisitor'] = allVisitor
    dictCtx['allSchool'] = allSchool
    dictCtx['allIntSchool'] = allInternalSchool

    tz901 = Tz901ComName.objects.filter(code=1,num=1).values("code_name2")
    if tz901.count() > 0:
        dictCtx['school1_name'] = '(' + tz901[0]['code_name2'] + ')'
    else:
        dictCtx['school1_name'] = '(内部S)'

    return dictCtx


def sub115_get_Ta215(tmpParam, tmpYear):

    tmpTableCtx = {}

    tmpQuery = ''
    if tmpParam == 'kamiki':
        tmpQuery = sub115_setSqlKamiki()
    elif tmpParam == 'simoki':
        tmpQuery = sub115_setSqlSimoki()
    else: #nenkan
        tmpQuery = sub115_setSqlNenkan()

    tmpQeryOption = " WHERE fiscal_end_year = %s "

    #ターゲットのデータ
    tmpSum = 0
    with connection.cursor() as cursor:
        cursor.execute(tmpQuery + tmpQeryOption, [tmpYear])
        ta215 = cursor.fetchall()

        for tmpTa215 in ta215:
            cYear = tmpTa215[0]
            cTotalMorning = tmpTa215[1] or 0
            cTotalAfternoon = tmpTa215[2] or 0
            cTotalNight = tmpTa215[3] or 0
            cTotalInternalSchool = tmpTa215[4] or 0
            cTotalSchool = tmpTa215[5] or 0
            cTotalMember = tmpTa215[6] or 0
            cTotalVisitor = tmpTa215[7] or 0
            cYearMonth = str(tmpTa215[8])

            tmpAll = cTotalSchool + cTotalMember + cTotalVisitor
            tmpSum += tmpAll

            tmpTableCtx.update({cYearMonth: {'business_ym':cYearMonth,
                                 'morning':cTotalMorning,
                                 'afternoon':cTotalAfternoon,
                                 'night':cTotalNight,
                                 'int_school':cTotalInternalSchool,
                                 'school':cTotalSchool,
                                 'member':cTotalMember,
                                 'visitor':cTotalVisitor,
                                 'all':tmpAll,
                                 'sum':tmpSum
                                 }})

    return tmpTableCtx


def sub115_setSqlNenkan():

    tmpQuery = " SELECT * " \
                " FROM  " \
                " (SELECT " \
                "    EXTRACT(YEAR FROM business_day)  AS fiscal_end_year, " \
                "    SUM(morning) AS total_morning, " \
                "    SUM(afternoon) AS total_afternoon, " \
                "    SUM(night) AS total_night, " \
                "    SUM(int_school) AS total_int_school, " \
                "    SUM(school_total) AS total_school, " \
                "    SUM(member) AS total_member, " \
                "    SUM(visitor) AS total_visitor, " \
                "    cast(EXTRACT(YEAR FROM business_day) as text) || to_char(EXTRACT(MONTH FROM business_day), 'FM00') AS fiscal_end_ym,  " \
                "    EXTRACT(MONTH FROM business_day) AS fiscal_end_month " \
                " FROM " \
                "    gf.ta215_attnd " \
                "GROUP BY " \
                "    fiscal_end_year, fiscal_end_ym, fiscal_end_month " \
                "ORDER BY " \
                "    fiscal_end_ym) as temp " 
    
    return tmpQuery

def sub115_setSqlKamiki():

    return sub115_setSqlHalfYear(HALF_YEAR_KAMIKI)

def sub115_setSqlSimoki():

    return sub115_setSqlHalfYear(HALF_YEAR_SIMOKI)


def sub115_setSqlHalfYear(pHalf):
    """上期・下期の来場者集計SQLを組み立てる（対象月と年度境界は設定から導出）"""
    fiscalYear = com_get_fiscal_year_sql('business_day')
    months = ', '.join(str(m) for m in com_get_half_year_months(pHalf))

    tmpQuery = " SELECT * " \
                " FROM  " \
                " (SELECT " \
                f"    {fiscalYear} AS fiscal_end_year, " \
                "    SUM(morning) AS total_morning, " \
                "    SUM(afternoon) AS total_afternoon, " \
                "    SUM(night) AS total_night, " \
                "    SUM(int_school) AS total_int_school, " \
                "    SUM(school_total) AS total_school, " \
                "    SUM(member) AS total_member, " \
                "    SUM(visitor) AS total_visitor, " \
                "    cast(EXTRACT(YEAR FROM business_day) as text) || to_char(EXTRACT(MONTH FROM business_day), 'FM00') AS fiscal_end_ym,  " \
                "    EXTRACT(MONTH FROM business_day) AS fiscal_end_month " \
                " FROM " \
                "    gf.ta215_attnd " \
                "WHERE " \
                f"    EXTRACT(MONTH FROM business_day) IN ({months}) " \
                "GROUP BY " \
                "    fiscal_end_year, fiscal_end_ym, fiscal_end_month " \
                "ORDER BY " \
                "    fiscal_end_ym) as temp "

    return tmpQuery





